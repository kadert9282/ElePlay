...
class UiStateStorage(context: Context) {
    ...
    companion object {
        ...
        private const val KEY_PLAYLIST_UPLOADER = "playlist_uploader"
        private const val KEY_PLAYLIST_COUNT = "playlist_count"
        private const val KEY_PLAYLIST_URL = "playlist_url"
        private const val KEY_PLAYLIST_SUMMARY = "playlist_summary"
        private const val KEY_PLAYLIST_SECTION_EXPANDED = "playlist_section_expanded"
    }
    ...
    fun savePlaylistPreview(
        ...
        prefs.edit()
            .putString(KEY_PLAYLIST_TITLE, title)
            .putString(KEY_PLAYLIST_UPLOADER, uploader)
            .putInt(KEY_PLAYLIST_COUNT, count)
            .putString(KEY_PLAYLIST_URL, url)
            .putString(KEY_PLAYLIST_SUMMARY, summary)
            .apply()
    }

    fun getPlaylistTitle(): String? {
        return prefs.getString(KEY_PLAYLIST_TITLE, null)
    }

    fun getPlaylistUploader(): String? {
        return prefs.getString(KEY_PLAYLIST_UPLOADER, null)
    }

    fun getPlaylistCount(): Int {
        return prefs.getInt(KEY_PLAYLIST_COUNT, 0)
    }
    ...
    fun getPlaylistSummary(): String? {
        return prefs.getString(KEY_PLAYLIST_SUMMARY, null)
    }

    fun savePlaylistSectionExpanded(expanded: Boolean) {
        prefs.edit().putBoolean(KEY_PLAYLIST_SECTION_EXPANDED, expanded).apply()
    }

    fun isPlaylistSectionExpanded(): Boolean {
        return prefs.getBoolean(KEY_PLAYLIST_SECTION_EXPANDED, true)
    }

    fun clearPlaylistPreview() {
        prefs.edit()
            .remove(KEY_PLAYLIST_TITLE)
            .remove(KEY_PLAYLIST_UPLOADER)
            .remove(KEY_PLAYLIST_COUNT)
            .remove(KEY_PLAYLIST_URL)
            .remove(KEY_PLAYLIST_SUMMARY)
            .apply()
    }
