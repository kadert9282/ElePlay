...
class DownloadForegroundService : Service() {
    ...
    companion object {
        ...
        const val EXTRA_RESULT_FILE_NAME = "extra_result_file_name"
        ...
        @Volatile
        var lastSavedUriString: String? = null
            private set
    ...
    private fun startDownload(intent: Intent) {
        if (downloadJob?.isActive == true || isRunning || finishing) {
            logger.log("Download already running")
            return
        }
        ...
        downloadJob = serviceScope.launch {
            try {
                ...
                if (!finishing && currentState != STATE_CANCELLED) {
    ...
    private fun handleProgress(progress: DownloadProgress, desiredName: String) {
        ...
        when (progress.phase) {
            DownloadProgress.Phase.COMPLETE -> finishSuccess(desiredName)
            DownloadProgress.Phase.ERROR -> finishError(progress.status.ifBlank { "Download failed" })
            DownloadProgress.Phase.CANCELLED -> finishCancelled()
            else -> Unit
        }
    }

    private fun finishSuccess(desiredName: String) {
        if (finishing) return
        finishing = true

        val lastFile = engine.getLastDownloadedFile()
        if (lastFile == null) {
            finishError("Downloaded file not found")
            return
        }

        val file = File(lastFile)
        if (!file.exists()) {
            finishError("Downloaded file missing on disk")
            return
        }

        val savedUri = saveToPreferredLocation(file, desiredName)
        if (savedUri == null) {
            finishError("Failed to save downloaded file")
            return
        }
    ...
    private fun finishError(message: String) {
        if (currentState == STATE_ERROR && finishing) return
        finishing = true
    ...
    private fun finishCancelled() {
        if (currentState == STATE_CANCELLED && finishing) return
        finishing = true
    ...
    private fun broadcastState(state: String, message: String) {
        val intent = Intent(ACTION_STATE_BROADCAST).apply {
            ...
            putExtra(EXTRA_RESULT_URI, lastSavedUriString)
            putExtra(EXTRA_RESULT_FILE_NAME, lastSavedFileName)
        }
        sendBroadcast(intent)
    }
