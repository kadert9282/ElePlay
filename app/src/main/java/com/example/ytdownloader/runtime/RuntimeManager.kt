...
class RuntimeManager(
    ...
    fun markRuntimeReady() {
        File(paths.readyMarker).writeText("ready_v_ffmpeg_final_node_qjs")
        logger.log("Runtime marked as ready")
    }
    ...
    suspend fun extractScripts() = withContext(Dispatchers.IO) {
        ...
        copyAssetDir("scripts", File(paths.scriptsDir))
        logger.log("yt_runner.py: ${File(paths.ytRunnerScript).exists()}, ${File(paths.ytRunnerScript).length()} bytes")
